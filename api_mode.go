...
// for backend to frontend communication for tool updates, etc. This is a simple broadcast hub that allows multiple clients to connect and receive messages.
type Hub struct {
	mu      sync.RWMutex
	writeMu sync.Mutex
...
func (s *Server) handleChat(w http.ResponseWriter, r *http.Request) {
	...
	message_len := s.chat.MessageCount()
	log.Printf("Chat request complete for session %s, total messages: %d", body.SessionID, message_len)
	...
	lastMsg := s.chat.GetRecentMessages(1)
